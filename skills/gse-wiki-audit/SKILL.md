...
name: gse-wiki-audit
description: >
  Audit the Job 824 wiki for health, consistency, sourcing, and mesh quality. Fork of gsewiki:audit
  plus two 824-specific checks (ultraplan 3.5): the whole-tree backlink sweep and the map-vs-disk
  consistency check.
---

# gse-wiki audit (824 fork)

Read `references/house-rules.md` first. Respect `_Memory/AGENTS.md`.

## Scope — upstream checks, house-adjusted

Frontmatter completeness/staleness · index/log coverage · duplicates/overlaps · stale claims · weak sourcing · contradictions/open-questions hygiene · promotion candidates in `_Memory/outputs/` · density ~40% **relevance-gated** (dense AND relevant; over-linking is a finding too). **House difference:** intentional ghost nodes (shared people/entity nodes) are a supported feature, NOT broken links — flag only unintentional phantoms (formal policy pending P5.3). Seed-page audit: project facts outside a `## 824 — project decisions` overlay heading are a finding (harvest separability, 3.3).

## 824-specific checks (the fork's additions)

1. **Whole-tree backlink sweep (D9):** every synthesis file under `Claude/` (drawing-db views, specs-md, reviews, extracts, map views) carries a `**Wiki:** [[...]]` backlink that resolves. The 461/461 sweep is the proven model — report count/count.
