...
name: gse-wiki-query
description: >
  Answer questions from the Job 824 wiki (Claude/_Memory/). Fork of gsewiki:query — map-aware:
...
4. **Map fallback (the 824 upgrade):** if the wiki lacks the answer, consult `Claude/Map/views/FILE_MAP.md` / `UNPROCESSED.md` before declaring ignorance — the answer may exist in a mapped-but-unprocessed document. Report that honestly: name the document and offer to ingest it (do not auto-process; Gavin's rule).
5. Drawing-derived claims: confirm the current set via `SUPERSEDED.md` + subproject context; cite the firm (original EOR / PK / MKM / G3).
